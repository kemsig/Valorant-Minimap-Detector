...
def write_to_image(main_image, asset_image_path, vari_x=0, vari_y=0, vari_w=0, vari_h=0, is_gadget_labeled=False):
    # Load the asset
...
def generate_data_YOLO(name, main_img_path, out_image_path, out_label_path, bb_variance=True, variance_pixel_range=5, gadgets=True, gadget_labeled=True):
    # open image
    ...
    for class_name, int_value in classes.items():
        ...
        for count in range(int_value):
            if 'gadget' in class_name:
                is_label_saved = False
            ...
            if bb_variance:
                x, y, width, height = write_to_image(
                    ...
                    is_gadget_labeled=is_label_saved
                )
            else:
                x, y, width, height = write_to_image(
                    main_image=main_img,
...
if __name__ == "__main__":
    generate_yolo_batch(count=5, name='daug_lotus_var5px_gadgets_no_label', background_path='Data Augmentation\\Backgrounds', output_path='TrainingSet\\DAUG_YOLO_SET')