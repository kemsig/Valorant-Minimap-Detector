...
def overlay_map_background(name, map_img_path, output_path, tp_image_path='Data Augmentation\\Backgrounds\\BG_TRANSPARENT\\TP_LOTUS.png'):
    ...
    # save result
    cv2.imwrite(os.path.join(output_path, f'{name}.jpg'), result)

def get_backgrounds(n):
    for i in range(n):
        map_path = get_rand_img(folder_name='',
                            root_path='TrainingSet\\Images',
                            exclusive=True,
                            file_type='jpg')
    overlay_map_background(f'daug_lotus_{i}', map_path, 'Data Augmentation\\Backgrounds')
    print(f'success {i}')
...
output_path = 'Data Augmentation\\gadget_output'
